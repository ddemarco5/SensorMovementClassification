...
public class DecisionStump implements Classifier, Regressor, Parameterized
{
    ...
    public static PairedReturn<Integer, Double> threshholdSplit(final Distribution dist1, final Distribution dist2)
    {
        ...
        double minRange = min(dist1.mean(), dist2.mean());
        double maxRange = max(dist1.mean(), dist2.mean());
        ...
        {
            minRange = min(dist1.invCdf(almost0), dist2.invCdf(almost0));
            maxRange = max(dist1.invCdf(almost1), dist2.invCdf(almost1));
            
            split = Zeroin.root(1e-8, minRange, maxRange, f, 0.0);
        }
        
        
        double minStnd = min(dist1.standardDeviation(), dist2.standardDeviation());
