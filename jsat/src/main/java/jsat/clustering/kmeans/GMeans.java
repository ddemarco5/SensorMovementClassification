...
public class GMeans extends KMeans 
{
    ...
    {
        return cluster(dataSet, 1, max(dataSet.getSampleSize()/20, 10), designations);
    }

    @Override
    public int[] cluster(DataSet dataSet, ExecutorService threadpool, int[] designations)
    {
        return cluster(dataSet, 1, max(dataSet.getSampleSize()/20, 10), threadpool, designations);
    }
    ...
    public int[] cluster(DataSet dataSet, int lowK, int highK, ExecutorService threadpool, int[] designations)
    {
        ...
        do
        {
            ...
            for(int c = 0; c < origMeans; c++)
            {
                ...
                Xp.mutableDivide(max(Xp.standardDeviation(), 1e-6));
